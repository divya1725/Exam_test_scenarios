        ...
        stage('ReadyAPITest') { 
            steps {
               ...
               script{
              	   ...
              	   // sh "./run-tests.sh"
                    sh """docker run -v="${WORKSPACE}/FraudPayments":/project -v="${WORKSPACE}/FraudPayments/reports":/reports -v="${WORKSPACE}/ext":/ext/ -e LICENSE_SERVER="fslicense.evry.com:1099" -e COMMAND_LINE="-f/FraudPayments/reports '-RJUnit-Style HTML Report' -FHTML '-E${params.Environments}' '/%project%/' '-s${params.suite}'"  fsnexus.evry.com:8085/smartbear/ready-api-soapui-testrunner:3.1.0"""
