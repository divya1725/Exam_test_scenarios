...
class TestEnvironment {
    ...
    public static void add( userReqEnvName , context ){
        def project = context.testCase.testSuite.project;
        int apiCount = project.getInterfaceCount();
        boolean envExists =  checkIfEnvExists(userReqEnvName, project);
        if (envExists.equals(false)){ 
            userReqEnvName = createNewTestEnvironment( userReqEnvName , apiCount , project ); 
        }
        if (apiCount != '0'){
            setEndPoint( userReqEnvName , apiCount , project );
        }
    ...
    public static void addWsdlInterface(activeEnv, counter, project){
        ...
        println "URL: "+urlDefinition;
        def wsdlName = wsdl.getName().toString();
        println "wsdlName: "+wsdlName;
    ...
    public static void setEndPoint( env , apiCount , project ){
        ...
        for ( int i = 0; i < apiCount ; i++ ){
            ...
            println wsdlInterface+" _ "+wsdlService;
            ...
            if(setupString != null){
                ...
                if ( urlConf != null ){
                    println "urlConf not null";
                    urlConf.setStringValue( setupString );
                }
            }
        }
    }
