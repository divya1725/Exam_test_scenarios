SELECT M.MESSAGEID,
  ...
  M.MARKETTYPE,
  DEBTOR_ACCOUNT.OTHER_ID AS DEBTACC,
  M.AGREEMENTID,
  M.INITIATORID,
  TO_CHAR(M.CREATIONDATETIME,'yyyy-mm-dd hh24:mi:ss') CREATEDDATE,
  TO_CHAR(T.RELEASEDATE, 'yyyy-mm-dd hh24:mi:ss') RELEASEDATE,
  T.STATUSCHANGETIME,
  T.DEBIT_BOOKING_ID,
  SE.BOOKINGDATE,
  T.PAYMENT_REF,
  ...
  T.CREDITTEXTCODE,
  M.ORIGINALFORMAT,
  P.CATEGORYPURPOSECODE,--M.CUSTOMER_TYPE,
  T.PAYMENTORDERDATE,
  T.CREDITAMOUNTAMT,
  T.CREDITAMOUNTCCY,
  T.EQUIVALENTAMOUNT          AS EQAMT,
  T.EQUIVALENTAMOUNT_CCYOFTRF AS EQAMTTRACCY,
  T.EQUIVALENTAMOUNTCCY       AS EQAMCCY,
  T.INSTRUCTEDAMOUNT,
  T.INSTRUCTEDAMOUNTCCY,
  T.DEBITCOUNTERVALUEAMT,
  T.MARKETPRODUCTCHARGEAMT,
  T.RISKSTATUS,
  ...
  DEBTOR_ACCOUNT.IBAN AS DEBTIBAN,
  DEBTOR_AGENT.NAME   AS DEBTAGENTNAME,
  DEBTOR_AGENT.BICFI  AS DEBTBIC,
  CREDITOR_PARTY.NAME CREDNAME,
  T.HIDDEN,
  T.EXCHANGERATEVALUE,
  CREDITOR_PARTY.COUNTRY  AS CREDCOUNTRY,
  CREDIT_ACCOUNT.IBAN     AS CREDACCIBAN,
  CREDIT_ACCOUNT.OTHER_ID AS CREDACCBBAN,
  CREDITOR_AGENT.BICFI    AS CREDAGTBIC,
  CREDITOR_AGENT.NAME     AS CREDAGENTNAME,
  T.EXCHANGERATE,
  T.EXCHANGERATETYPE,
  CREDITOR_AGENT.MEMBERID CREDMEMBERID,
  CREDITOR_AGENT.CLEARINGSYSTEMID CREDCLEARID,
  T.DEBITAMOUNTAMT,
  T.DEBITAMOUNTCCY,
  RES.RESERVATIONREFERENCE,
  TO_CHAR(P.REQUESTEDEXECUTIONDATE, 'yyyy-mm-dd hh24:mi:ss') REQEXECDATE
FROM TRANSACTIONS T
INNER JOIN PAYMENTINFO P
ON T.FK_PAYMENTINFOID = P.PAYMENTINFOID
INNER JOIN MESSAGE M
ON M.MESSAGEID = P.FK_MESSAGEID
LEFT OUTER JOIN AGENT_PAYMENTINFO DEBTOR_AGENT
ON DEBTOR_AGENT.FK_PAYMENTINFOID = P.PAYMENTINFOID
AND DEBTOR_AGENT.FK_AGENTTYPEID  =3
LEFT OUTER JOIN AGENT_TRANS CREDITOR_AGENT
ON CREDITOR_AGENT.FK_TRANSACTIONSID = T.TRANSACTIONSID
AND CREDITOR_AGENT.FK_AGENTTYPEID   =2
LEFT OUTER JOIN PARTY_DEBTOR DEBTOR_PARTY
ON DEBTOR_PARTY.FK_PAYMENTINFOID = P.PAYMENTINFOID
AND DEBTOR_PARTY.FK_PARTYTYPEID  =2
LEFT OUTER JOIN PARTY_CREDITOR CREDITOR_PARTY
ON CREDITOR_PARTY.FK_TRANSACTIONSID= T.TRANSACTIONSID
AND CREDITOR_PARTY.FK_PARTYTYPEID  =3
LEFT OUTER JOIN ACCOUNT_DEBTOR DEBTOR_ACCOUNT
ON DEBTOR_ACCOUNT.FK_PAYMENTINFOID = P.PAYMENTINFOID
AND DEBTOR_ACCOUNT.FK_ACCOUNTTYPEID=1
LEFT OUTER JOIN ACCOUNT_CREDITOR CREDIT_ACCOUNT
ON CREDIT_ACCOUNT.FK_TRANSACTIONSID = T.TRANSACTIONSID
AND CREDIT_ACCOUNT.FK_ACCOUNTTYPEID =3
LEFT OUTER JOIN REGULATORYREPORTING REGREP
ON REGREP.FK_TRANSACTIONSID = T.TRANSACTIONSID
LEFT OUTER JOIN REGULATORYREPORTING_DTLS REGREPDET
ON REGREPDET.FK_RGLTRYRPTGID = REGREP.RGLTRYRPTGID
LEFT OUTER JOIN RESERVATION_DETAILS RES
ON RES.FK_TRANSACTIONSID = T.TRANSACTIONSID
AND RES.RESERVATION_TYPE ='D'
LEFT OUTER JOIN STATUSREASON S
ON S.FK_TRANSACTIONID = T.TRANSACTIONSID
LEFT OUTER JOIN SETTLEMENT SE
ON SE.FK_TRANSACTIONSID = T.TRANSACTIONSID
LEFT OUTER JOIN STATUSREASON_ADDT_INF SA
ON SA.FK_STATUSREASONID = S.STATUSREASONID
WHERE 1                 =1
  --AND M.INITIATORBANKID  IN ('3625')
  --AND T.ENDTOENDID LIKE ('BM_PROFXR%')
  --AND TRUNC(M.CREATIONDATETIME) > TRUNC(SYSDATE-2)
AND T.TRANSACTIONSID IN ('81436')
  --ORDER BY PAYMENTSTATUS DESC,T.TRANSACTIONSID DESC;
  --AND T.PAYMENTSTATUS NOT IN ('VALI','TBAP','TBVE','CANC','REJC')  --'VALI','TBAP','TBVE','RFCA','','','','',
  --AND M.MARKETTYPE='BM'
  --AND T.PAYMENTTYPE ='UNDE'
  --AND T.TRANSACTIONSID LIKE ('35621%')
  --AND MESSAGEID IN ('2865300')
  --AND T.FK_PROCESSINGLINEID = '3'
  ...
  --AND DEBTOR_ACCOUNT.OTHER_ID IN('42022049911')
